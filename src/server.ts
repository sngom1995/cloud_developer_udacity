...
(async () => {
  ...
  app.get("/cars/:make", async (req: Request, res:Response) => {
    ...
    if (make){
      cars = cars_list.filter((car)=> car.make === make);
      return res.status(200)
                .send(cars);
    }
    return res.status(200)
  ...
  app.post("/cars", async (req: Request, res:Response)=>{
      ...
      if(!id || !type || !model || !cost){
        res.status(404).send("model, make , id, type, and cost are required");
      }

      const  new_car: Car =  {id: model, type: type, make: make, model: model, cost: cost}
